...
@Component({
  selector: 'app-asistencia-aero-esq',
  templateUrl: './asistencia-aero-esq.component.html',
  styleUrls: ['./asistencia-aero-esq.component.css']
})
export class AsistenciaAeroEsqComponent implements OnInit {
  readonly NOMBRE_TABLA = 'asistencia_aero_esq';
  reglas: any[] = [];
  ...
  onFileChange(event: any) {
    ...
    reader.onload = (e: any) => {
      try {
        ...
        // Usamos índice 0, asumiendo que es la primera hoja como en Calimaya
        const ws = workbook.Sheets[workbook.SheetNames[0]]; 
        const rawData: any[][] = XLSX.utils.sheet_to_json(ws, { header: 1, defval: "" });
        
        this.dataExcelMemoria = this.mapearExcel(rawData);
        this.notificaciones.mostrar(`Aero-Esq: ${this.dataExcelMemoria.length} marcajes listos`, 'exito');
      } catch (err) {
  ...
  generarProcesamiento() {
    ...
    this.dataExcelMemoria.forEach(reg => {
      ...
      if (emp && this.diasParaTabla.includes(reg.fecha)) {
        if (!emp.asistencias[reg.fecha]) {
          emp.asistencias[reg.fecha] = reg.hora;
          if (reg.hora > emp.reglaInicio && reg.hora <= emp.reglaFin) {
            emp.retardos[reg.fecha] = reg.hora;
          } else if (reg.hora > emp.reglaFin) {
            emp.limites[reg.fecha] = reg.hora;
          }
        }
      }
    });
    ...
    this.notificaciones.mostrar('Análisis completado: Aero Esquina', 'exito');
  }
  ...
  exportarExcelResultados() {
    const element = document.getElementById('tabla-aero-esq-resultados');
    const ws = XLSX.utils.table_to_sheet(element);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Reporte');
    XLSX.writeFile(wb, `Asistencia_Aero_Esquina.xlsx`);
  }
