...
@Component({
  selector: 'app-asistencia-aero-hac',
  templateUrl: './asistencia-aero-hac.component.html',
  styleUrls: ['./asistencia-aero-hac.component.css']
})
export class AsistenciaAeroHacComponent implements OnInit {
  readonly NOMBRE_TABLA = 'asistencia_aero_hac';
  reglas: any[] = [];
  ...
  onFileChange(event: any) {
    ...
    reader.onload = (e: any) => {
      try {
        ...
        this.notificaciones.mostrar(`Aero-Hac: ${this.dataExcelMemoria.length} marcajes listos`, 'exito');
      } catch (err) {
  ...
  private mapearExcel(rawData: any[][]): any[] {
    ...
    for (let i = 0; i < rawData.length; i++) {
      ...
      if (fila[0]?.toString().trim().toUpperCase() === "ID:") {
        empleadoActual = fila[4]?.toString().trim(); 
        filaIdIdx = i;
        continue;
      }
  ...
  generarProcesamiento() {
    ...
    this.notificaciones.mostrar('Análisis completado: Aero Hacienda', 'exito');
  }
  ...
  exportarExcelResultados() {
    const element = document.getElementById('tabla-aero-hac-resultados');
    const ws = XLSX.utils.table_to_sheet(element);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Reporte');
    XLSX.writeFile(wb, `Asistencia_Aero_Hacienda.xlsx`);
  }
